...
- Призовой фонд 50 000 рублей (на первые 5 призовых мест) + дополнительный приз зрительских симпатий!
...
- Имеются 4 дополнения: Royalty, Ideology, Biotech и Odyssey (все кроме Anomaly)
...
- Весь процесс прохождения должен проходить в прямом эфире на [Twitch](https://www.twitch.tv/) или [YouTube](https://www.youtube.com/) (а лучше и там, и там) с тегами #RuinShip #RuinShipTournament
...
### Быстрее всего в папку с другими файлами можно попасть через 'Открыть сохранения' в настройках Rimworld
...
**Файл 'Турнир RuinShip.rsc' (Сценарий турнира) поместить по этому пути**
...
Удачи в вашем прохождении и помните, что самое главное - это путь (даже если без караванов), так что попробуйте не заруинить свой корабль и победить мехрой за эти 30 дней и у вас обязательно всё получится!(или нет<a:AnimatedA_PeepoNoob:982560159974965308>) Мы верим в вас и в вашу победную! Точно так же, как стример верит в свою!<a:AnimatedA_ThisIsFine:982563794133852220>
			...
			self.embed(
				description = """
## P.S.
По любым вопросам можете обращаться к людям с ролью <@&1241274685459529789>, а можете просто пингануть эту роль в канале <#1401143860519899178>.
				"""
			),
		]
		...
		for embed in embeds:
			if isinstance(embed, list):
				await webhook.send("", embeds=embed, username="RuinShip")
			else:
				await webhook.send("", embed=embed, username="RuinShip")
